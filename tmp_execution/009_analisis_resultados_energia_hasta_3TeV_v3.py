...
# %%
list_runs=new_create_main_list_runs([6,40],chose_runs)#new_create_main_list_runs([2,6,6,6,6,6,6],chose_runs)
...
for i,y in enumerate(y_pred_all):
    ...
    plt.tight_layout()
    plt.savefig(f"{BASE_DIR}/results/009_correlation_energy_filter_{i}_v3.png")
    plt.clf()
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(f"{BASE_DIR}/results/009_resid_hist_energy_filter_{i}_v3.png")
    plt.clf()
    plt.close(fig2)
