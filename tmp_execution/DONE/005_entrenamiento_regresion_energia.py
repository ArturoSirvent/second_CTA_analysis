...
basedir="/home/asirvent/second_CTA_analysis"
npy_final_dir=f"{basedir}/datos/elementos_npy"
base_dir_elementos=f"{basedir}/datos/elementos"
...
for i,arch in enumerate(opciones_filtros):
    ...
    with open(f"{basedir}/automat/logs/{file_number}_data_control_energy.txt","a") as registro:
        registro.write(f"Con arquitectura: {arch}, y uso de CPU  ?? Mb, y memoria {get_all_size(list(locals().items()))} Mb \n")
    ...
    for k in range(n):
        ...
        print(f"\n Boostrap {k+1} de {n}, y uso de CPU  ?? Mb, y memoria {get_all_size(list(locals().items()))} Mb \n")

        list_runs=new_create_main_list_runs([7,25],chose_runs)#new_create_main_list_runs([2,6,6,6,6,6,6],chose_runs)
        with open(f"{basedir}/automat/logs/{file_number}_data_control_energy.txt","a") as registro:
            registro.write(f"Boostrap {k+1} de {n},runs: {list_runs}, y uso de CPU  ?? Mb, y memoria {get_all_size(list(locals().items()))} Mb \n")
        ...
        hist=modelo.fit(x=x_train_list,y=y_train_list,epochs=21, validation_data=(x_test_list,y_test_list),batch_size=32)
        ...
        modelo.save(f"{basedir}/modelos/{file_number}_modelo_filtro_{i}_en_boostrap_stage_{k+1}_energy.h5")
        with open(f"{basedir}/modelos/performances/{file_number}_history_modelo_filtro_{i}_en_boostrap_stage_{k+1}_energy.pickle","wb") as pick:
