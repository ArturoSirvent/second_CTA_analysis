...
for i,arch in enumerate(opciones_filtros):
    ...
    for k in range(n):
        ...
        list_runs=new_create_main_list_runs([1,2,2,2,2,2,2],chose_runs)
        with open(f"../automat/logs/{file_number}_data_control.txt","a") as registro:
            registro.write(f"Boostrap {k+1} de {n},runs: {list_runs},y uso memoria CPU antes carga datos: {tf.config.experimental.get_memory_info('CPU:0')['current']>>20}Mb \t")
        ...
        with open(f"../automat/logs/{file_number}_data_control.txt","a") as registro:
                    registro.write(f"Y uso memoria CPU después carga datos: {tf.config.experimental.get_memory_info('CPU:0')['current']>>20}Mb \n")
